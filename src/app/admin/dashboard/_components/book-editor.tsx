...
export function BookEditor({ book, isCreating, categories, onSave, onCancel }: BookEditorProps) {
  ...
  const { reset, handleSubmit, formState: { isSubmitting, isDirty } } = form;
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      if (name === 'title' && isCreating) {
        form.setValue('slug', generateSlug(value.title || ''), { shouldValidate: true, shouldDirty: true });
      }
    });
      ...
      <Form {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 h-full flex flex-col">
            <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                <h2 className="text-2xl font-semibold">{isCreating ? 'Create New Book' : 'Edit Book'}</h2>
                ...
                <FormField
                    ...
                    <FormItem>
                        ...
                        <FormControl>
                        <Input placeholder="the-great-gatsby" {...field} disabled={!isCreating} />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                    )}
                ...
                <FormField
                    ...
                    <FormItem>
                        ...
                        <FormControl>
                        <Textarea placeholder="A novel about the American dream..." className="min-h-[150px]" {...field} />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                    )}
